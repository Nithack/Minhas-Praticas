...
function contaVitorias2(listaCampeoes, nome) {
    let result = 0
    for (let index = 0; index < listaCampeoes.length; index++) {
        if (listaCampeoes[index].country == nome) {
            result++
        }
...
function contaVitorias(listaCampeoes, nome) {
    let result = 0
    listaCampeoes.map((element) => {
        if (element.country == nome) result++
    })
    return result
}